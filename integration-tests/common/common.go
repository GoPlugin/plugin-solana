...
import (
	...
	"github.com/goplugin/plugin-testing-framework/k8s/pkg/helm/sol"

	"github.com/goplugin/plugin/integration-tests/client"
	"github.com/goplugin/plugin/integration-tests/docker/test_env"

	"github.com/goplugin/plugin/v2/core/services/job"
	"github.com/goplugin/plugin/v2/core/store/models"

	chainConfig "github.com/goplugin/plugin-solana/integration-tests/config"
...
func CreateBridges(ContractsIdxMapToContractsNodeInfo map[int]*ContractNodeInfo, mockURL string, isK8s bool) error {
	for i, nodesInfo := range ContractsIdxMapToContractsNodeInfo {
		...
		if isK8s {
			nodeContractPairID, err = BuildNodeContractPairID(nodesInfo.BootstrapNodeK8s.PluginClient, nodesInfo.OCR2.Address())
		} else {
		...
		for j := 0; j < nodeCount; j++ {
			var clClient *client.PluginClient
			if isK8s {
				clClient = nodesInfo.NodesK8s[j].PluginClient
			} else {
...
func BuildNodeContractPairID(node *client.PluginClient, ocr2Addr string) (string, error) {
