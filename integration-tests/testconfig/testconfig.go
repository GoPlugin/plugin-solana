...
import (
	...
	ocr2_config "github.com/goplugin/plugin-solana/integration-tests/testconfig/ocr2"
...
func (c *TestConfig) ReadFromEnvVar() error {
	...
	pluginImage := ctf_config.MustReadEnvVar_String(ctf_config.E2E_TEST_PLUGIN_IMAGE_ENV)
	if pluginImage != "" {
		if c.PluginImage == nil {
			c.PluginImage = &ctf_config.PluginImageConfig{}
		}
		logger.Info().Msgf("Using %s env var to override PluginImage.Image", ctf_config.E2E_TEST_PLUGIN_IMAGE_ENV)
		c.PluginImage.Image = &pluginImage
	}

	pluginUpgradeImage := ctf_config.MustReadEnvVar_String(ctf_config.E2E_TEST_PLUGIN_UPGRADE_IMAGE_ENV)
	if pluginUpgradeImage != "" {
		if c.PluginUpgradeImage == nil {
			c.PluginUpgradeImage = &ctf_config.PluginImageConfig{}
		}
		logger.Info().Msgf("Using %s env var to override PluginUpgradeImage.Image", ctf_config.E2E_TEST_PLUGIN_UPGRADE_IMAGE_ENV)
...
func (c *Common) Validate() error {
	...
	err = os.Setenv("PLUGIN_ENV_USER", *c.User)
	if err != nil {
		return fmt.Errorf("could not set PLUGIN_ENV_USER env var")
	}
