...
import { Round as OCRRound, OCR2Feed } from "@plugin/solana-sdk";
...
describe("ocr2", () => {
  ...
  it("Creates the PLI token", async () => {
  ...
  it("Creates the token vault", async () => {
    ...
    // Create an associated token account for PLI, owned by the program instance
    tokenVault = (
  ...
  let configure = async () => {
    ...
    // Fund the owner with PLI tokens
    await mintTo(
      provider.connection,
      fromWallet,
      token,
